...
async def amount_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.reply_text(f"Co masz za towar? (1/{amount})")
...
async def confirm_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in USER_LIMITS:
        ...
        if now - first < DAY_SECONDS:
            if count >= DAILY_LIMIT:
                await q.message.reply_text("⛔ Limit 2 ogłoszeń na 24h wykorzystany.")
                return
        else:
    ...
    contact = f"@{q.from_user.username}" if q.from_user.username else "brak_username"
